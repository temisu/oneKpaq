...
;; end of preproc and setup, start of real stuff
	...
	;
	; inputs:
	; bx concatenated block1+block2, pointer to start of block2
	; di dest (must be zero filled and writable from -13 byte offset
...
onekpaq_decompressor:
	...
	lea si,[byte di-(9+4)]	; si=dest, di=window start
	lodsd
	inc eax
	mov ecx,eax

%ifdef ONEKPAQ_DECOMPRESSOR_MULTI_SECTION
	lea dx,[byte bx+1]		; header=src-3 (src has -4 offset)
%else
	lea dx,[byte bx+3]		; header=src-1 (src has -4 offset)
%endif
	; ebp unitialized, will be cleaned by following loop + first decode
	; which will result into 0 bit, before actual data

.normalize_loop:
	shl byte [byte bx+4],1
	jnz short .src_byte_has_data
	inc bx
	rcl byte [byte bx+4],1		; CF==1
.src_byte_has_data:
	rcl ebp,1
...
.weight_upload_loop:
.shift:	equ $+2
	rol dword [bx],byte ONEKPAQ_DECOMPRESSOR_SHIFT
	fidivr dword [bx]
	mov [bx],edx
